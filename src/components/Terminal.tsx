    ...
    if (lower === "help") {
      ...
      response = (
        <div className="space-y-2 text-xs">
          ...
          <p className="text-slate-600 dark:text-slate-300">{PERSONAL_INFO.secondaryBio}</p>
        </div>
      );
            ...
            <div key={i}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {cat.skills.map((s, idx) => (
                  <span
                    key={idx}
                    className="px-2 py-0.5 rounded bg-white dark:bg-slate-800 text-sky-700 dark:text-sky-300 border border-slate-200 dark:border-slate-700 font-mono text-[11px]"
                  >
                    {s.name} <span className="text-slate-500 dark:text-slate-400">({s.level})</span>
                  </span>
                ))}
              </div>
            </div>
      ...
      response = (
        <div className="space-y-2.5 text-xs">
          {PROJECTS.map((p, idx) => (
            <div key={idx} className="p-3 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700">
              ...
              <p className="text-slate-600 dark:text-slate-300 text-[11px] mt-1">{p.description}</p>
              <div className="flex flex-wrap gap-1.5 mt-2">
                {p.tags.map((t, tIdx) => (
                  <span key={tIdx} className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200/80 dark:border-transparent">
                    {t}
                  </span>
                ))}
              </div>
      ...
      response = (
        <div className="space-y-3 text-xs">
          {EXPERIENCES.map((exp, idx) => (
            <div key={idx} className="p-3.5 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">{exp.role} @ {exp.company}</span>
                <span className="text-[11px] text-slate-500 dark:text-slate-400">{exp.period}</span>
              </div>
              <div className="space-y-2 pl-1">
                {exp.projects.map((proj, pIdx) => (
                  <div key={pIdx} className="text-slate-700 dark:text-slate-300 space-y-1">
                    <div className="font-bold text-sky-700 dark:text-sky-300">• {proj.name} <span className="text-slate-500 dark:text-slate-400 font-normal">({proj.subtitle})</span></div>
                    <ul className="list-disc list-inside space-y-0.5 pl-2 text-[11px] text-slate-600 dark:text-slate-300">
      ...
      response = (
        <div className="space-y-2 text-xs">
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700">
            ...
            <div className="text-slate-700 dark:text-slate-200 mt-0.5">Higher Secondary Certificate (HSC) – Science Stream</div>
      ...
      response = (
        <div className="text-xs space-y-1.5 text-slate-700 dark:text-slate-300 p-3 rounded-xl bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
      ...
      response = (
        <div className="text-xs space-y-2 text-slate-700 dark:text-slate-300 p-3 rounded-xl bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700">
          <p className="text-emerald-600 dark:text-emerald-400 font-bold text-sm">📄 Hetu Patel — Full Stack &amp; Mobile Developer</p>
          <p className="text-slate-600 dark:text-slate-300">{PERSONAL_INFO.summary}</p>
      ...
      response = <div className="text-xs text-slate-700 dark:text-slate-200">{trimmed.substring(5)}</div>;
    } else if (lower === "sudo") {
      response = <div className="text-xs text-rose-500 dark:text-rose-400">Permission denied: You are a guest visitor, but feel free to hire Hetu! 😉</div>;
    } else {
      response = (
  ...
  return (
    <section id="terminal" className="py-16 sm:py-24 relative">
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 xl:px-16">
        ...
        <div className="rounded-3xl border border-slate-200/90 dark:border-slate-800/90 bg-white dark:bg-[#090d16] text-slate-800 dark:text-slate-200 overflow-hidden font-mono text-xs sm:text-sm transition-colors duration-300">
          {/* Top Title Bar */}
          <div className="bg-slate-100/90 dark:bg-[#0f172a] px-4 py-3 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-600 dark:text-slate-400 text-xs font-medium ml-2 hidden sm:inline">
                hetu@portfolio:~ (bash)
              </span>
            </div>
            <div className="flex items-center gap-2 text-[11px] text-slate-500 dark:text-slate-400">
              <button
                onClick={() => setLogs([])}
                className="hover:text-slate-900 dark:hover:text-white flex items-center gap-1 transition-colors px-2.5 py-1 rounded-lg bg-white dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700"
              >
          ...
          <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-900/80 border-b border-slate-200 dark:border-slate-800/80 flex items-center gap-2 overflow-x-auto scrollbar-none">
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-sans font-semibold shrink-0">
              Quick commands:
            </span>
            {COMMAND_SUGGESTIONS.map((cmd) => (
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-white hover:bg-sky-50 hover:border-sky-300 hover:text-sky-700 dark:bg-slate-800 dark:hover:bg-sky-500/20 dark:hover:border-sky-500/50 dark:hover:text-sky-300 border border-slate-200/90 dark:border-slate-700 text-[11px] text-slate-700 dark:text-slate-300 font-mono transition-colors shrink-0"
              >
                {cmd}
              </button>
            ))}
          ...
          <div ref={contentRef} className="p-4 sm:p-5 max-h-[380px] min-h-[220px] overflow-y-auto space-y-3.5 bg-[#fafbfc] dark:bg-[#090d16] transition-colors duration-300">
            {logs.map((log, idx) => (
              <div key={idx} className="space-y-1.5">
                {log.command && (
                  <div className="flex items-center gap-2">
                    <span className="text-emerald-600 dark:text-emerald-400 font-bold">hetu@dev:~$</span>
                    <span className="font-semibold text-slate-900 dark:text-slate-100">{log.command}</span>
                  </div>
                )}
          ...
          <div className="px-4 py-3 bg-slate-100/90 dark:bg-[#060a12] border-t border-slate-200 dark:border-slate-800 flex items-center gap-2">
            ...
            <input
              ...
              className="w-full bg-transparent border-none outline-none text-slate-900 dark:text-slate-100 font-mono text-xs sm:text-sm placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:ring-0"
            />
            <button
